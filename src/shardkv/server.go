...
func (kv *ShardKV) CommitOp(op Op) {
	if op.Op == "Put" {
		...
		kv.Table[op.Shard][op.Key] += op.Value
		fmt.Printf("shardserver:%v append result:%v\n", kv.logHead, kv.Table[op.Shard][op.Key])
		...
		for k, v := range op.Table { //table为空就不做操作了
			kv.Table[k] = v
		}
...
func (kv *ShardKV) Get(args *GetArgs, reply *GetReply) {
	...
	if kv.configs[len(kv.configs)-1].Shards[key2shard(args.Key)] != kv.gid {
...
func (kv *ShardKV) PutAppend(args *PutAppendArgs, reply *PutAppendReply) {
	...
	if args.Retry {
		if kv.rf.SearchLog(op, op.Ts) == raft.AlreadyCommited {
			...
			return
		} else if kv.rf.SearchLog(op, op.Ts) == raft.NotCommited {
...
func StartServer(servers []*labrpc.ClientEnd, me int, persister *raft.Persister, maxraftstate int, gid int, masters []*labrpc.ClientEnd, make_end func(string) *labrpc.ClientEnd) *ShardKV {
	...
	fmt.Printf("shardserver:%vsnapshot\n", kv.logHead)
	...
	kv.logHead = strconv.Itoa(kv.gid) + "_" + strconv.Itoa(kv.me)
	kv.configInit = false
...
func (kv *ShardKV) makeTableFromSnapshot() {
	...
	if d.Decode(&index) != nil ||
		...
		d.Decode(&valid) != nil { //这个table直接这么搞也不知道好不好使
...
func (kv *ShardKV) pullShard(groups map[int][]string, shardsPull []int) map[int]map[string]string {
	...
	*/
	...
	for k, _ := range groups {
		if k != kv.gid {
			go kv.pullShardRpc(groups, k, shardsPull, gidChan[k]) //shard全发过去了
		}
	}
...
func (kv *ShardKV) QueryConfigRoutine() {
	for {
		...
		if !kv.configInit {
			...
			if len(gidShard) > 0 {
				shardData = kv.pullShard(gidShard)
			}
		}
